...
class DailySummaryGenerator:
    ...
    def _generate_html_content(self, data: Dict[str, Any]) -> str:
        ...
        html.append(
            ...
            f"<tr><td>Imp avoided: € {data.get('d_savings').get('total_avoided_cost_eur'):.2f} "
            f"({data.get('d_savings').get('total_export_kwh'):.2f} kWh)</td>"
            f"<td>Imp avoided: € {data.get('m_savings').get('total_avoided_cost_eur'):.2f} "
            f"({data.get('m_savings').get('total_export_kwh'):.2f} kWh)</td>"
            f"<td>Imp avoided: € {data.get('y_savings').get('total_avoided_cost_eur'):.2f} "
            f"({data.get('y_savings').get('total_export_kwh'):.2f} kWh)</td></tr>"
            f"<tr><td>Exp missed: € {data.get('d_savings').get('total_opportunity_cost_eur'):.2f} "
            f"({data.get('d_savings').get('total_import_kwh'):.2f} kWh)</td>"
            f"<td>Exp missed: € {data.get('m_savings').get('total_opportunity_cost_eur'):.2f} "
            f"({data.get('m_savings').get('total_import_kwh'):.2f} kWh)</td>"
            f"<td>Exp missed: € {data.get('y_savings').get('total_opportunity_cost_eur'):.2f} "
